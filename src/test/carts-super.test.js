...
describe('Get All Carts', () => {
    it('should get all carts', async () => {
        ...
        console.log("Get All Carts Response:", response.body);
...
describe('Carts testing , CRUD operations', () => {
    ...
    it('should create a cart', async () => {
        ...
        console.log("Create Cart Response:", response.body);
        expect(response.body).to.be.an('object');
    ...
    it('should update a cart with a product and a quantity', async () => {
        ...
        const response = await requester.put(`/api/carts/${cartsId}`).send(cartMock);
        console.log("Update Cart Response:", response.body);
        expect(response.body).to.be.an('object');
    ...
    it('should get a specific product by ID', async () => {
        ...
        console.log("Get Cart by ID Response:", response.body);
        expect(response.body).to.be.an('object');
        expect(response.body).to.have.property('cart');
    });

    it('should remove one product from the cart', async () => {
        const response = await requester.delete(`/api/carts/${cartsId}/products/65b19ebc3250bf4167b53ab2`);
        console.log("Remove Product from Cart Response:", response.body);
        expect(response.body).to.be.an('object');
    ...
    it('should update quantity of the product', async () => {
        ...
        const response = await requester.put(`/api/carts/${cartsId}/products/65b19f183250bf4167b53ab6`).send(updateProductQuantity);
        console.log("Update Product Quantity Response:", response.body);
        expect(response.body).to.be.an('object');
        expect(response.body).to.have.property('products');
    ...
    it('should empty the cart', async () => {
        const response = await requester.delete(`/api/carts/${cartsId}`);
        console.log("Empty Cart Response:", response.body);
